...
mod integration_tests {
    ...
    fn integration_test_get_order_by_user() {
        ...
        let order_to_create2 = CreateOrder {
            ...
            orderlines: vec![Orderline {item_num: 1, price: 5}],
        };
        ...
        workers::create_order(Json(order_to_create1.clone()), &ip).unwrap();
        std::thread::sleep(std::time::Duration::from_secs(5));
        workers::create_order(Json(order_to_create2.clone()), &ip).unwrap();
        std::thread::sleep(std::time::Duration::from_secs(5));
        workers::create_order(Json(order_to_create3.clone()), &ip).unwrap();
        std::thread::sleep(std::time::Duration::from_secs(5));
