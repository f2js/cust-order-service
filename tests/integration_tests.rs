...
use order_service::{api::workers::{self, create_table}, models::orders::{CreateOrder, Orderline}};

macro_rules! start_container_and_create_table {
    ($docker: expr) => {{
        ...
        match res {
            Ok(_) => {Ok((hbase, ip))},
            Err(e) => {println!("Error!: {:?}", e.to_string()); Err(e)},
        }
    }}
}
...
fn test_get_tables() {
    ...
    let res = match workers::get_tables(&ip) {
        Ok(r) => r,
        Err(e) => {println!("Error!: {:?}", e.to_string()); panic!("Booooo")},
    };
    for table in res {
...
fn test_create_order_empty() {
    ...
    let rowkey = workers::create_order(Json(order_to_create.clone()), &ip).unwrap();
...
fn test_create_order() {
    ...
    for ol in res.orderlines.iter() {
        assert!(ol.item_num == ol1.item_num || ol.item_num == ol2.item_num || ol.item_num == ol3.item_num);
